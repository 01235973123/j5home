...
$cols = count($fields) + 4;
?>
<div id="osm-subscription-history" class="osm-container osm-container-j4">
	<?php
	...
	<form method="post" name="adminForm" id="adminForm" action="<?php echo Route::_('index.php?option=com_osmembership&view=groupmembersimples&Itemid=' . $this->Itemid); ?>">
		...
		<table class="<?php echo $this->bootstrapHelper->getClassMapping('table table-striped table-bordered table-hover'); ?>">
			...
			<tbody>
				...
				for ($i = 0, $n = count($this->items); $i < $n; $i++) {
					$row  = $this->items[$i];
					$link = Route::_('index.php?option=com_osmembership&view=groupmembersimple&id=' . $row->id . '&Itemid=' . $this->Itemid);
				?>
					<tr>
						<td>
