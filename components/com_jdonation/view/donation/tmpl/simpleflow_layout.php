...
$activate_form_float = false;
if(DonationHelper::isJoomla4() && $this->config->twitter_bootstrap_version == '5' && $this->config->activate_form_floating)
{
	$activate_form_float = true;
	$floating_css = "form-floating";
}
...
{
	background-color: #<?php echo ($color != '') ? $color : 'b250d2' ?> !important;
}
#donation-form .switch-payment-gateway input:checked + label 
{
	border:1px solid #<?php echo ($color != '') ? $color : 'b250d2' ?> !important;
}
...
?>
<div id="donation-form" class="<?php echo $rowFluidClass;?> jd-container donationsimple <?php echo $extralayoutCss; ?>">
    ...
    if($allow_donation)
    {
        if($this->campaign->id > 0 && $this->config->show_campaign == 1)
        {
            ...
            <div class="<?php echo $rowFluidClass?>">
                <div class="<?php echo $span12Class;?> campaigndescription" id="donation_form">
                    <h3 class="jd-page-title"><?php echo Text::_('JD_DONATION'); ?></h3>
            <?php
        }
		else 
		{
            ...
            ?>
            <h<?php echo $h?> class="jd-page-title" id="donation_form"><?php echo Text::_('JD_DONATION'); ?></h<?php echo $h?>>
            <?php
        }
        //show campaign
        if($this->campaign->id > 0)
            ...
            <div class="<?php echo $rowFluidClass?>">
                <div class="<?php echo $span12Class; ?>" >
                    <div class="jd_display_table jd_float_left subheading">
                        ...
                        <h4 class="jd_display_table_cell jd_vertical_align_middle nd_options_color_greydark jd_padding_0_responsive jd_padding_left_20 jd_margin_0 jd_padding_top_0"><?php echo strtoupper(Text::_('JD_CHOOSE_SELECT_AMOUNTS'));?></h4>
                    </div>
                </div>
            </div>
            <div class="<?php echo $rowFluidClass?>">
                <div class="<?php echo $span12Class; ?>" id="amount_container">
                    ...
                    if ($this->config->donation_amounts)
                    {
                        ...
                        {
                            $options = [];
                            $options[] = HTMLHelper::_('select.option', '', Text::_('JD_AMOUNT')) ;
                            for ($i = 0 , $n = count($amounts) ; $i < $n ; $i++)
                            {
                                $amount = $amounts[$i] ;
								if(strpos($amount, "[c]") > 0)
                                ...
                                if (isset($explanations[$i]) && $explanations[$i])
                                {
                                    $options[] = HTMLHelper::_('select.option', $amount, DonationHelperHtml::formatAmount($this->config, $amount)." [$explanations[$i]]") ;
                                }
                                else
                                {
                                    $options[] = HTMLHelper::_('select.option', $amount, DonationHelperHtml::formatAmount($this->config, $amount)) ;
                                }
                            }
                            echo  '<span id="jd_currency_symbol">'.$this->config->currency_symbol.'</span>  '.HTMLHelper::_('select.genericlist', $options, 'rd_amount', ' class="'.$extraValidateClass.' '.$inputLargeClass.' form-select uk-select" onchange="clearTextbox();" ', 'value', 'text', $this->rdAmount).'<br /><br />';
                        }
                    }
                    ?>
                    <div id="otherAmount">
		...
		if ($this->config->enable_recurring)
		{
			if ($this->campaignId)
			{
				...
				if($this->campaign->donation_type == 2 && $this->method->getEnableRecurring())
				{
					$checkbox_showing = "style='display:none;'";
				}
			}
			else
			{
		...
		?>
			...
			<?php
				...
				?>
				<div class="simple-recurring-row" id="donation_type" <?php echo $style1; ?>>
                    <input type="checkbox" name="donation_type" class="form-check-input" value="recurring" id="donation_typerecurring" <?php echo $checked;?> <?php echo $checkbox_showing;?>/>
                    <?php echo Text::_('JD_MAKE_THIS_DONATION_EVERY');?>
                    <?php
                    if (count((array)$this->recurringFrequencies) > 1)
                    {
                        echo $this->lists['r_frequency1'];
                    }
                    else
                    {
                    ...
                    ?>
                        ...
                        echo Text::_('JD_IN');
                        ?>
                        <input type="text" name="r_times" id="r_times" value="<?php echo $this->input->getInt('r_times', null); ?>" class="<?php echo $inputSmallClass;?>"/>

                        <?php
                        echo Text::_('JD_TIMES');
                        ?>
		...
		else
		{
            ...
            if (($this->config->registration_integration == 1 || $this->config->registration_integration == 2) && $this->config->show_login_box)
			{
                ...
                $return     = base64_encode(Route::_('index.php?option=com_jdonation&view=donation'.$campaign_variable.'&Itemid='.$itemid));
                $link       = Route::_('index.php?option=com_users&view=login&return='.$return);
                $msg       .= " <a target='_blank' class='btn btn-primary uk-button uk-button-primary loginbutton' href='".$link."' title='".Text::_('JD_CLICK_HERE_TO_LOGIN')."'>".Text::_('JD_LOGIN')."</a>";
            }
        }
        ?>
        <div class="<?php echo $rowFluidClass?>">
        ...
        ?>
            ...
            if ($this->config->use_campaign)
            {
                ...
                else
                {
                    ...
                    <div class="<?php echo $span12Class?> campaignTitleDiv">
                        <label class="sbjtitle"><?php echo Text::_('JD_CAMPAIGN');?>&nbsp;<?php echo $this->campaign->title; ?></label>
                    </div>
					<?php
					}	
					?>
			...
			$array = ['Text','Textarea','Date','List','State','Countries'];
            foreach ($fields as $field)
            {
				...
				if($visible)
				{
					...
					if($field->name == 'comment' || $field->type == 'Message')
					{
						...
						<div class="<?php echo $rowFluidClass?>">
							<div class="<?php echo $span12Class ?> donationfields <?php echo $floating_css; ?>">
								<?php
								echo $field->getControlGroupSimple(false, $bootstrapHelper, $field);
								
								if($activate_form_float)
								{
								?>
					...
					else 
					{
						...
						?>
						<div class="<?php echo $span6Class ?> donationfields <?php echo $extra_style;?> <?php echo $formGroupClass;?> <?php echo $floating_css; ?>" id="<?php echo $stateIDfield;?>">
							<?php
							echo $field->getControlGroupSimple(false, $bootstrapHelper, $field);
							if($activate_form_float && in_array($field->type, $array))
							{
								if($field->name == "state")
								{
								?>
									<label for="state"><?php echo $field->title;?></label>
								<?php
								}
								else
								{
            ...
            {
            ?>
                <div class="<?php echo $rowFluidClass;?> hide_me">
					<div class="<?php echo $span12Class;?>">
						<label class="hasTooltip hasTip" for="hide_me" title="<?php echo Text::_('JD_HIDE_DONOR_EXPLAIN');?>">
							<input type="checkbox" class="form-check-input" name="hide_me" value="1" <?php if ($this->hideMe) echo ' checked="checked"' ; ?> />
            ...
            {
            ?>
                <div class="<?php echo $rowFluidClass;?> gift_aid">
					<div class="<?php echo $span12Class;?>">
						<label class="hasTooltip hasTip" for="gift_aid" title="<?php echo Text::_('JD_GIFT_AID_EXPLAIN');?>">
							<input type="checkbox" name="gift_aid" class="form-check-input" value="1" <?php if ($this->gift_aid) echo ' checked="checked"' ; ?> />
							<?php echo Text::_('JD_GIFT_AID_EXPLAIN');?>
						</label>
					</div>
                </div>
            <?php
            ...
            if($this->show_dedicate == 1)
            {
                ...
                <div class="<?php echo $rowFluidClass?>" id="dedicate_heading">
                    <div class="<?php echo $span12Class?> <?php echo $formGroupClass;?>">
                        <input type="checkbox" class="form-check-input" name="show_dedicate" id="show_dedicate" value="0" onclick="javascript:showDedicate();"/>
                        <label for="show_dedicate" id="label_for_show_dedicate"><?php echo Text::_('JD_HONOR_OF'); ?></label>
                    </div>
                </div>
                    ...
                    <div class="<?php echo $span12Class?>">
                        ...
                        <div class="<?php echo $rowFluidClass?>">
                            <div class="<?php echo $span6Class?> <?php echo $formGroupClass;?>">
                                <input type="text" class="jd_width_100_percentage <?php echo $formControlClass;?>" name="dedicate_name" value="" placeholder="<?php echo Text::_('JD_HONOREE_NAME'); ?>" />
                            </div>
                            <div class="<?php echo $span6Class?> <?php echo $formGroupClass;?>">
                                <input type="text" class="jd_width_100_percentage <?php echo $formControlClass;?>" name="dedicate_email" value="" placeholder="<?php echo Text::_('JD_HONOREE_EMAIL'); ?>" />
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="<?php echo $controlGroupClass;?>">
                <h3 class="jd-heading"><?php echo Text::_('JD_DONATION_INFORMATION'); ?></h3>
            </div>
            -->
            ...
            <div class="<?php echo $rowFluidClass?>">
                <div class="<?php echo $span12Class; ?>" >
                    <div class="jd_display_table jd_float_left subheading">
                        ...
                        <h4 class="jd_display_table_cell jd_vertical_align_middle nd_options_color_greydark jd_padding_0_responsive jd_padding_left_20 jd_margin_0 jd_padding_top_0"><?php echo strtoupper(Text::_('JD_CHECKOUT'));?></h4>
                    </div>
                </div>
            </div>
            ...
            <?php
                ...
                if ($this->config->pay_payment_gateway_fee)
                {
                    ...
                    <div class="<?php echo $controlGroupClass;?>" id="pay_payment_gateway_fee_div">
                        <label class="<?php echo $controlLabelClass;?> sbjtitle" for="pay_payment_gateway_fee">
                            <?php echo  Text::_('JD_PAY_PAYMENT_GATEWAY_FEE'); ?>
                        </label>
                        <?php
                    ...
                    <div class="<?php echo $rowFluidClass?> paymentmethodtitle">
                        <div class="<?php echo $span12Class; ?>" >
							<label class="sbjtitle">
								<?php echo Text::_('JD_SELECT_PAYMENT_OPTION'); ?>
							</label>
                        </div>
                    </div>
                ...
                if ($this->config->show_newsletter_subscription == 1 && DonationHelper::isNewsletterPluginEnabled())
				{
                    ...
                    <div class="<?php echo $controlGroupClass;?>">
                        ...
                        <div class="<?php echo $controlsClass;?>">
                            <input type="checkbox" name="newsletter_subscription" class="form-check-input" value="1" id="newsletter_subscription" />
                        </div>
                    </div>
                    <?php
                }
                if ($this->config->accept_term ==1 && $this->config->article_id > 0)
                {
                    ...
                    <div class="<?php echo $controlGroupClass;?>">
                        <label class="checkbox">
                            <input type="checkbox" name="accept_term" class="form-check-input" value="1" class="validate[required]" data-errormessage="<?php echo Text::_('JD_ACCEPT_TERMS');?>" />
                    ...
                    <div class="<?php echo $controlGroupClass ?>">
                        <div class="<?php echo $controlLabelClass; ?>">
                            ...
                            if ($link)
                            {
                                $extra = ' jd-modal' ;
                            ?>
                                <a href="<?php echo $link; ?>"  class="eb-colorbox-privacy-policy <?php echo $extra; ?>"><?php echo Text::_('JD_PRIVACY_POLICY');?></a>
                            <?php
                            }
                            else
                            {
                        ...
                        <div class="<?php echo $controlsClass; ?>">
                            <input type="checkbox" name="agree_privacy_policy" value="1" class="validate[required] form-check-input" data-errormessage="<?php echo Text::_('JD_AGREE_PRIVACY_POLICY_ERROR');?>" />
                            <?php
                            $agreePrivacyPolicyMessage = Text::_('JD_AGREE_PRIVACY_POLICY_MESSAGE');
            ...
            <div class="<?php echo $rowFluidClass;?>">
				<div class="<?php echo $span12Class; ?> donationButton">
					<input type="submit" class="btn btn-primary uk-button uk-button-primary" name="btnSubmit" id="btn-submit" value="<?php echo  Text::_('JD_PROCESS_DONATION') ;?>" />
				</div>
            </div>
            <?php
                if (count($this->methods) == 1)
...
for ($j = 0 , $m = count($rowCampaigns) ; $j < $m ; $j++)
{
    ...
    <div id="campaign_recurring_frequency_<?php echo $campaign->id; ?>" style="display: none;">
        ...
        if(($campaign->donation_type == 0 || $campaign->donation_type == 2) && $campaign->recurring_frequencies != "")
        {
            ...
            echo HTMLHelper::_('select.genericlist', $options, 'r_frequency', ' class="input-medium validate[required]" ', 'value', 'text', Factory::getApplication()->input->get('r_frequency', '')) ;
        }
        ?>
    </div>
    <?php
    ...
    for ($j = 0 , $m = count($rowCampaigns) ; $j < $m ; $j++)
    {
        ...
        <div id="campaign_<?php echo $rowCampaign->id; ?>" style="display: none;" class="<?php echo $rowFluidClass?>">
            <div class="<?php echo $span12Class?>">
                ...
                ?>
					<div class="switch-amounts <?php echo $rowFluidClass?>">
						...
						for ($i = 0 , $n = count($amounts) ; $i < $n ; $i++)
						{
							...
							?>
							<input type="radio" name="rd_amount" id="pre_amount_<?php echo $i; ?>" value="<?php echo $amount; ?>" <?php echo $checked ; ?> onclick="clearTextbox();" class="<?php echo $extraClass; ?>" />
							<label for="pre_amount_<?php echo $i; ?>">
								<?php echo ' '.DonationHelperHtml::formatAmount($this->config, $amount);?>
								<?php
								if (isset($explanations[$i]) && $explanations[$i])
                ...
                {
                    $options = [];
                    $options[] = HTMLHelper::_('select.option', '', Text::_('JD_DONATION_AMOUNT')) ;
                    for ($i = 0 , $n = count($amounts) ; $i < $n ; $i++)
                    {
                        $amount = (float)$amounts[$i] ;
                        if ($amount == $this->rdAmount)
                    ...
                    echo  '<span id="jd_currency_symbol">'.$this->config->currency_symbol.'</span>  '.HTMLHelper::_('select.genericlist', $options, 'rd_amount', ' class="'.$extraClass.' '.$inputLargeClass.' form-select uk-select" onchange="clearTextbox();" ', 'value', 'text', $this->rdAmount).'<br /><br />';
                }
                ?>
                <div class="<?php echo $rowFluidClass?>">
                    <?php
...
<div style="display:none;" id="amount_container_backup">
    ...
    if ($this->config->donation_amounts)
    {
                ...
                for ($i = 0 , $n = count($amounts) ; $i < $n ; $i++)
                {
                    ...
                    <div class="<?php echo $span;?> jd_padding_bottom_20_desktop jd_padding_top_10" style="<?php echo $extra_style;?>">
                        <div class="jd_predefined_box jd_width_100_percentage jd_float_left jd_padding_0_responsive jd_padding_bottom_20_important_responsive jd_padding_right_15 jd_box_sizing_border_box jd_width_100_percentage_responsive">
                            <input type="radio" name="rd_amount" class="<?php echo $extraValidateClass; ?>" value="<?php echo $amount; ?>" <?php echo $checked ; ?> onclick="clearTextbox();" data-errormessage="<?php echo Text::_('JD_AMOUNT_IS_REQUIRED'); ?>" /><?php echo ' '.DonationHelperHtml::formatAmount($this->config, $amount);?>
                            <?php
        ...
        else
        {
            ...
            echo  '<span id="jd_currency_symbol">'.$this->config->currency_symbol.'</span>  '.HTMLHelper::_('select.genericlist', $options, 'rd_amount', ' class="'.$extraValidateClass.'  '. $inputLargeClass.' form-select" onchange="clearTextbox();" ', 'value', 'text', $this->rdAmount).'<br /><br />';
        }
    }
    ?>
    <div class="<?php echo $rowFluidClass?>">
