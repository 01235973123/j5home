			...
			<div class="mainframe_search osp-container">
								...
								<ul class="nav nav-tabs">
									...
									?>
										<li <?php echo $active; ?>><a
												href="<?php echo Route::_('index.php?option=com_osproperty&view=lsearch&locator_type=' . $tid . '&Itemid=' . $jinput->getInt('Itemid', 0)) ?>"><?php echo $type_name; ?></a>
										</li>
									<?php
									}
									?>
				...
				<script type="text/javascript">
					...
					function updateLocatorPrice(type_id, live_site) {
						...
						url = live_site +
							"index.php?option=com_osproperty&no_html=1&tmpl=component&task=ajax_updatePrice&type_id=" + type_id +
							"&option_id=<?php echo $lists['price_value']; ?>&min_price=<?php echo $lists['min_price']; ?>&max_price=<?php echo $lists['max_price']; ?>&module_id=adv";
						...
						if (xmlHttp.readyState == 4 || xmlHttp.readyState == "complete") {
							...
							if (mod_osservice_price != null) {
								...
								Number.prototype.formatMoney = function(decPlaces, thouSeparator, decSeparator) {
									...
									return sign + (j ? i.substr(0, j) + thouSeparator : "") + i.substr(j).replace(
										/(\d{3})(?=\d)/g, "$1" + thouSeparator) + (decPlaces ? decSeparator + Math.abs(n -
										i).toFixed(decPlaces).slice(2) : "");
								};
							}
						}
					}
					...
					<div class="<?php echo $bootstrapHelper->getClassMapping('row-fluid'); ?>">
						<div class="<?php echo $class ?>" id="mapDiv">
							...
							<script type='text/javascript' defer='defer'
								src='<?php echo Uri::root() ?>media/com_osproperty/assets/js/markerclusterer.js'></script>
							<script type='text/javascript' defer='defer'
								src='<?php echo Uri::root() ?>media/com_osproperty/assets/js/markerwithlabel_packed.js'></script>

							<link rel="stylesheet" href="<?php echo Uri::root() ?>media/com_osproperty/assets/css/jquery-ui.css"
								type="text/css" />
							<script type="text/javascript">
								jQuery.noConflict();
								(function($) {
										jQuery(document).ready(function() {
											...
											var min = .999999;
											var max = 1.000001;
											var myHome = new google.maps.LatLng(<?php echo $search_lat; ?>,
												<?php echo $search_long ?>);
											...
											?>
													var propertyListing<?php echo $rows[$key]->id ?> = new google.maps.LatLng(
														<?php echo $rows[$key]->lat_add; ?>,
														<?php echo $rows[$key]->long_add; ?>);
											<?php
												}
											}
											?>
											...
											jQuery('#togglebtn').click(function() {
												if (jQuery("#mapDiv").hasClass(
														"<?php echo $bootstrapHelper->getClassMapping('span7'); ?>"
													)) {
													jQuery("#mapDiv").removeClass(
														"<?php echo $bootstrapHelper->getClassMapping('span7'); ?>"
													);
													jQuery("#mapDiv").addClass(
														"<?php echo $bootstrapHelper->getClassMapping('span12'); ?>"
													);
													...
													<?php } ?>
													jQuery('#togglebtn').empty().append(
														'<img src="<?php echo Uri::root(); ?>media/com_osproperty/assets/images/exit_full_screen.png" class="width25"  alt="<?php Text::_("OS_EXIT_FULL_SCREEN"); ?>"/>'
													);
												} else {
													jQuery("#mapDiv").removeClass(
														"<?php echo $bootstrapHelper->getClassMapping('span12'); ?>"
													...
													<?php } ?>
													jQuery('#togglebtn').empty().append(
														'<img src="<?php echo Uri::root(); ?>media/com_osproperty/assets/images/full_screen.png" class="width25"  alt="<?php Text::_("OS_FULL_SCREEN"); ?>"/>'
													);
												}
												return false;
											});
											...
											function makeMarker(options) {
												if (navigator.userAgent.match(/(iPod|iPhone|iPad)/)) {
													...
													var pushPin = new MarkerWithLabel({
														map: map
													});
												}
											...
											?>
													...
													if (count($addInfo) > 0) {
													?>
														contentString<?php echo $row->id ?> +=
															'<div class="ospitem-iconbkgr"><span class="ezitem-leftpad"><?php echo implode(" | ", $addInfo); ?></span></div>';
													<?php
													}
													?>
													contentString<?php echo $row->id ?> +=
														'<?php echo htmlspecialchars(str_replace("'", "\"", str_replace("\r", "", str_replace("\n", "", $row->pro_small_desc)))); ?> <a href="<?php echo Route::_("index.php?option=com_osproperty&task=property_details&id=" . $row->id . "&Itemid=" . $itemid) ?>"><?php echo Text::_('OS_DETAILS'); ?></a></p>' +
														'</div>' +
														'</div>';
													<?php
													...
													makeMarker({
														...
														icon: new google.maps.MarkerImage(
															'<?php echo Uri::root() ?>media/com_osproperty/assets/images/googlemapicons/<?php echo $type_icon; ?>'
														)
													});
												<?php
													// }
												...
												jQuery("#divitem<?php echo $i ?>").mouseover(function() {
													google.maps.event.trigger(markerArray[<?php echo $i ?>],
														'mouseover');
												})
											...
											jQuery(function() {
												jQuery("#location").autocomplete({
													...
													function makerOver(i) {
														google.maps.event.trigger(markerArray[i],
															'mouseover');
													}
							...
							?>
								<div id="toggle" class="gmapcontroller">
									<span id="togglebtn" class="gmapcontroller_fullscreen">
										<img src="<?php echo Uri::root(); ?>media/com_osproperty/assets/images/full_screen.png"
											style="width:25px !important;" alt="<?php Text::_('OS_FULL_SCREEN'); ?>" />
									</span>
								</div>
							<?php
							}
							...
							?>
								<div id="locator_map_notice" class="locator_map_notice hidden-phone">
									<strong><?php echo sprintf(Text::_('OS_ONLY_SHOW_PROPERTIES'), $configClass['max_locator_results']); ?></strong>
									<?php echo Text::_('OS_USE_FILTER_TO_NARROW_YOUR_SEARCH'); ?>
								</div>
							<?php } ?>
							<!-- End Notice -->
							<?php
							...
							?>
								...
								<div id="locator_sidebar" class="margintop30">
									<div id="locator_sidebar1"
										style="background-color: transparent; background-image: url('<?php echo Uri::root() ?>media/com_osproperty/assets/images/sidebg1.png');">
								...
								<script type="text/javascript">
									jQuery('#locator_hidebar, #locator_showbar').click(function() {
										jQuery('#locator_sidebar').toggle('slow');
										jQuery('#locator_showbar').toggle('slow')
									});
								</script>
							<?php } ?>
						...
						<?php if ($lists['location'] != "" && count($rows) > 0 && $style == "style1") {
						?>
							<div class="<?php echo $bootstrapHelper->getClassMapping('span5'); ?> <?php echo $bootstrapHelper->getClassMapping('hidden-phone'); ?>"
								id="listPropertiesDiv">
								<?php
								if (count($rows) > 0) {
								?>
										...
										<div id="listview">
											...
											?>
												<div class="<?php echo $bootstrapHelper->getClassMapping('row-fluid'); ?> locatormap_icon">
													<div class="<?php echo $bootstrapHelper->getClassMapping('span12'); ?> conten_e_property"
														id="divitem<?php echo $i ?>">
														...
														<strong>
															...
															?>
																<span id="compare<?php echo $row->id; ?>">
																	...
																	if (!OSPHelper::isInCompareList($row->id)) {
																		...
																		$msg = str_replace("'", "\'", $msg);
																	?>
																		<a onclick="javascript:osConfirmExtend('<?php echo $msg; ?>','ajax_addCompare','<?php echo $row->id ?>','<?php echo Uri::root() ?>','compare<?php echo $row->id; ?>','theme3','listing')"
																			href="javascript:void(0)">
																			<img title="<?php echo Text::_('OS_ADD_TO_COMPARE_LIST') ?>"
																				alt="<?php echo Text::_('OS_ADD_TO_COMPARE_LIST') ?>"
																				src="<?php echo Uri::root(true) ?>/media/com_osproperty/assets/images/compare24_gray.png"
																				border="0" width="16" /></a>
																</span>
															<?php
																	} else {
																		...
																		$msg = str_replace("'", "\'", $msg);
															?>
																<a onclick="javascript:osConfirmExtend('<?php echo $msg; ?>','ajax_removeCompare','<?php echo $row->id ?>','<?php echo Uri::root() ?>','compare<?php echo $row->id; ?>','theme3','listing')"
																	href="javascript:void(0)">
																	<img title="<?php echo Text::_('OS_ADD_TO_COMPARE_LIST') ?>"
																		alt="<?php echo Text::_('OS_REMOVE_FROM_COMPARE_LIST') ?>"
																		src="<?php echo Uri::root(true) ?>/media/com_osproperty/assets/images/compare24.png"
																		border="0" width="16" /></a>
																</span>
															<?php
																	}
															...
															?>
																	<span id="fav<?php echo $row->id; ?>">
																		<a onclick="javascript:osConfirmExtend('<?php echo $msg; ?>','ajax_addFavorites','<?php echo $row->id ?>','<?php echo Uri::root() ?>','fav<?php echo $row->id; ?>','theme3','listing')"
																			href="javascript:void(0)">
																			<img class="width16" title="<?php echo Text::_('OS_ADD_TO_FAVORITES') ?>"
																				alt="<?php echo Text::_('OS_ADD_TO_FAVORITES') ?>"
																				src="<?php echo Uri::root(true); ?>/media/com_osproperty/assets/images/save24_gray.png"
																				border="0" />
																		</a>
																	</span>
																<?php
																...
																?>
																	<span id="fav<?php echo $row->id; ?>">
																		<a onclick="javascript:osConfirmExtend('<?php echo $msg; ?>','ajax_removeFavorites','<?php echo $row->id ?>','<?php echo Uri::root() ?>','fav<?php echo $row->id; ?>','theme3','listing')"
																			href="javascript:void(0)">
																			<img class="width16"
																				title="<?php echo Text::_('OS_REMOVE_PROPERTY_OUT_OF_FAVORITES_LIST') ?>"
																				alt="<?php echo Text::_('OS_REMOVE_PROPERTY_OUT_OF_FAVORITES_LIST') ?>"
																				src="<?php echo Uri::root(true); ?>/media/com_osproperty/assets/images/save24.png"
																				border="0" />
																		</a>
																	</span>
														<?php
														...
														<span id="fav<?php echo $row->id; ?>">
															<a href="<?php echo $link; ?>"
																title="<?php echo Text::_('OS_VIEW_LISTING_DETAILS') ?>">
																<img title="<?php echo Text::_('OS_VIEW_LISTING_DETAILS') ?>"
																	alt="<?php echo Text::_('OS_VIEW_LISTING_DETAILS') ?>"
																	src="<?php echo Uri::root(true); ?>/media/com_osproperty/assets/images/details.png"
																	border="0" />
															</a>
														</span>
														</strong>
