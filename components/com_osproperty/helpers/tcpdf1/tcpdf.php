...
class TCPDF {
	...
	public function Image($file, $x='', $y='', $w=0, $h=0, $type='', $link='', $align='', $resize=false, $dpi=300, $palign='', $ismask=false, $imgmask=false, $border=0, $fitbox=false, $hidden=false, $fitonpage=false, $alt=false, $altimgs=array()) {
		...
		if (!empty($imgdata)) {
			// copy image to cache
			$original_file = $file;
			$file = TCPDF_STATIC::getObFilename('img', $this->file_id);
		...
		if ($newimage) {
			...
			if (($info === false) AND function_exists($gdfunction)) {
				try {
					...
					if ($img !== false) {
						...
						if (($type == 'gif') OR ($type == 'png')) {
							$info = TCPDF_IMAGES::_toPNG($img, TCPDF_STATIC::getObFilename('img', $this->file_id));
						} else {
							$info = TCPDF_IMAGES::_toJPEG($img, $this->jpeg_quality, TCPDF_STATIC::getObFilename('img', $this->file_id));
						}
					}
				} catch(Exception $e) {
					$info = false;
			...
			if (($info === false) AND extension_loaded('imagick')) {
				try {
					...
					$tempname = TCPDF_STATIC::getObFilename('img', $this->file_id);
	...
	public function Output($name='doc.pdf', $dest='I') {
		...
		if ($this->sign) {
			...
			$tempdoc = TCPDF_STATIC::getObFilename('doc', $this->file_id);
			...
			$tempsign = TCPDF_STATIC::getObFilename('sig', $this->file_id);
	...
	protected function _generateencryptionkey() {
		...
		if (!$this->encryptdata['pubkey']) { // standard mode
			...
			foreach ($this->encryptdata['pubkeys'] as $pubkey) {
				...
				$tempkeyfile = TCPDF_STATIC::getObFilename('key', $this->file_id);
				...
				fclose($f);
				$tempencfile = TCPDF_STATIC::getObFilename('enc', $this->file_id);
