...
{
	class OSPJPagination extends CMSObject
	{
		/**
		...
		function getPagesCounter()
		{
			...
			if ($this->get('pages.total') > 1) {
				$html .= Text::_('Page')." ".$this->get('pages.current')." ".Text::_('of')." ".$this->get('pages.total');
			}
			return $html;
		}
		...
		function getResultsCounter()
		{
			...
			if ($this->total > 0) {
				$msg = Text::sprintf('Results of', $fromResult, $toResult, $this->total);
				$html .= "\n".$msg;
			} else {
		...
		function getLimitBox()
		{
			...
			if(in_array($this->limit,array(10,15,20,30,50,100))){
				...
				$limits[] = HTMLHelper::_('select.option', 10);
				$limits[] = HTMLHelper::_('select.option', 15);
				$limits[] = HTMLHelper::_('select.option', 20);
				$limits[] = HTMLHelper::_('select.option', 30);
				...
				if ($mainframe->isAdmin()) {
					$html = HTMLHelper::_('select.genericlist',  $limits, 'limit', 'class="input-mini" size="1" onchange="submitform();"', 'value', 'text', $selected);
				} else {
					$html = HTMLHelper::_('select.genericlist',  $limits, 'limit', 'class="input-mini" size="1" onchange="document.ftForm.submit()"', 'value', 'text', $selected);
				}
				return $html;
			}else{
				return '';
		...
		function _list_footer($list)
		{
			...
			if ($lang->isRTL()) {
				...
				if($list['limitfield'] != ""){
					$html .= "\n<div class=\"limit\">".Text::_("OS_DISPLAY_NUM").$list['limitfield']."</div>";
				}
			} else {
				if($list['limitfield'] != ""){
					$html .= "\n<div class=\"limit\">".Text::_("OS_DISPLAY_NUM").$list['limitfield']."</div>";
		...
		function _item_active(&$item)
		{
			...
			}elseif(($item->text == Text::_("OS_END")) || ($item->text == Text::_("OS_START"))){
				$hiddenphone = "hidden-phone";
			}else{
				$hiddenphone = "";
			}
			...
			{
				$priority_task = array('company_listing','company_properties','property_advsearch','property_manageallproperties','agent_editprofile','agent_default','category_details','agent_listing','agent_layout','property_type','category_listing');
				$priority_view = array('lmanageproperties','aeditdetails','lagents','ladvsearch','ltype','lcompanies','lcategory');
			}
			else
			{

				$priority_task = array('company_listing','company_properties','property_advsearch','property_manageallproperties','agent_editprofile','agent_default','category_details','agent_listing','agent_layout');
				$priority_view = array('lmanageproperties','aeditdetails','lagents','ladvsearch','lcompanies');

			}


			if(in_array($task,$priority_task) || in_array($view,$priority_view))
			{
				if($item->base>0)
				{
		...
		function _buildDataObject()
		{
			...
			$data->all	= new OSPJPaginationObject(Text::_('OS_VIEW_ALL'));
			if (!$this->_viewall) {
				$data->all->base	= '0';
				$data->all->link	= Route::_("&limitstart=");
			}
	
			// Set the start and previous data objects
			$data->start	= new OSPJPaginationObject(Text::_("OS_START"));
			$data->previous	= new OSPJPaginationObject(Text::_("OS_PREVIOUS"));
			...
			if ($this->get('pages.current') > 1)
			{
				...
				$data->start->base	= '0';
				$data->start->link	= Route::_("&limitstart=0");
				$data->previous->base	= $page;
				$data->previous->link	= Route::_("&limitstart=".$page);
			}
	
			// Set the next and end data objects
			$data->next	= new OSPJPaginationObject(Text::_("OS_NEXT"));
			$data->end	= new OSPJPaginationObject(Text::_("OS_END"));
			...
			if ($this->get('pages.current') < $this->get('pages.total'))
			{
				...
				$data->next->base	= $next;
				$data->next->link	= Route::_("&limitstart=".$next);
				$data->end->base	= $end;
				$data->end->link	= Route::_("&limitstart=".$end);
			}
			...
			for ($i = $this->get('pages.start'); $i <= $stop; $i ++)
			{
				...
				if ($i != $this->get('pages.current') || $this->_viewall)
				{
					...
					$data->pages[$i]->link	= Route::_("&limitstart=".$offset);
				}
			}
			return $data;
		}
	...
	if (!class_exists('JPaginationClone')){
		class JPaginationClone extends CMSObject
		{
			/**
			 * The record number to start dislpaying from
			 *
			...
			function getPagesCounter()
			{
				...
				$html = null;
				if ($this->get('pages.total') > 1) {
					$html .= Text::_('Page')." ".$this->get('pages.current')." ".Text::_('OS_OF')." ".$this->get('pages.total');
				}
				return $html;
			}
			...
			function getLimitBox()
			{
				...
				$limits[] = HTMLHelper::_('select.option', '50');
				$limits[] = HTMLHelper::_('select.option', '100');
				$limits[] = HTMLHelper::_('select.option', '0', Text::_('all'));
				...
				if($mainframe->isClient('administrator'))
				{
					$html = HTMLHelper::_('select.genericlist',  $limits, 'limit', 'class="input-mini form-select" size="1" onchange="submitform();"', 'value', 'text', $selected);
				} 
				else 
			{
					$html = HTMLHelper::_('select.genericlist',  $limits, 'limit', 'class="input-mini form-select" size="1" onchange="document.ftForm1.submit()"', 'value', 'text', $selected);
				}
				return $html;
			}
			...
			function _list_footer($list)
			{
				...
				if ($lang->isRTL()) {
					$html .= "\n<div class=\"counter\">".$list['pagescounter']."</div>";
					$html .= $list['pageslinks'];
					$html .= "\n<div class=\"limit\">".Text::_("Display num").$list['limitfield']."</div>";
				} else {
					$html .= "\n<div class=\"limit\">".Text::_("Display num").$list['limitfield']."</div>";
					$html .= $list['pageslinks'];
					$html .= "\n<div class=\"counter\">".$list['pagescounter']."</div>";
			...
			function _buildDataObject()
			{
				...
				$data->all	= new OSPOSPJPaginationObject(Text::_('View All'));
				if (!$this->_viewall) {
					$data->all->base	= '0';
					$data->all->link	= Route::_("&limitstart=");
				}
				...
				// Set the start and previous data objects
				$data->start	= new OSPJPaginationObject(Text::_("Start"));
				$data->previous	= new OSPJPaginationObject(Text::_("Previous"));
				...
				if ($this->get('pages.current') > 1)
				{
					...
					$data->start->base	= '0';
					$data->start->link	= Route::_("&limitstart=");
					$data->previous->base	= $page;
					$data->previous->link	= Route::_("&limitstart=".$page);
				}
		
				// Set the next and end data objects
				$data->next	= new OSPJPaginationObject(Text::_("Next"));
				$data->end	= new OSPJPaginationObject(Text::_("End"));
	...
	if (!class_exists('OSPJPaginationObject')){
		class OSPJPaginationObject extends CMSObject
		{
			var $text;
			var $base;
			var $link;
