...
<style>
	...
	@media (min-width: 1280px) {
		.os_property-item .status-type:after {
			border-right: 9px solid <?php echo $titleColor;
									?> !important;
		}
...
<div class="<?php echo $bootstrapHelper->getClassMapping('row-fluid'); ?>" id="propertydetails">
	<d class="<?php echo $bootstrapHelper->getClassMapping('span12'); ?>">
		<div class="lightGrad detailsView clearfix">
			<div class="<?php echo $bootstrapHelper->getClassMapping('row-fluid'); ?>">
				...
				if ($language->isRTL()) {
					$csssuffix = "_rtl";
				?>
					<div class="<?php echo $bootstrapHelper->getClassMapping('span12'); ?> property-title"
						style="border-right:10px solid <?php echo $titleColor; ?>;">
					<?php
				} else {
					?>
						<div class="<?php echo $bootstrapHelper->getClassMapping('span12'); ?> property-title"
							style="border-left:10px solid <?php echo $titleColor; ?>;">
						<?php
					}
						?>
						<h1>
							...
							if ($configClass['enable_report'] == 1) {
								OSPHelperJquery::colorbox('a.reportmodal');
								?>
								<a href="<?php echo Uri::root() ?>index.php?option=com_osproperty&tmpl=component&item_type=0&task=property_reportForm&id=<?php echo $row->id ?>"
									class="reportmodal reportlink" rel="{handler: 'iframe', size: {x: 350, y: 600}}"
									title="<?php echo Text::_('OS_REPORT_LISTING'); ?>">
						...
						<span>
							...
							?>
								<span class="badge badge-warning"><strong><?php echo Text::_('OS_SOLD') ?></strong></span>
								<?php echo Text::_('OS_ON'); ?>: <?php echo $row->soldOn; ?>
							...
							<div class="<?php echo $bootstrapHelper->getClassMapping('row-fluid'); ?>">
								<div class="<?php echo $bootstrapHelper->getClassMapping('span7'); ?>">
									...
									?>
										...
										<script type="text/javascript"
											src="<?php echo Uri::root(true) ?>/media/com_osproperty/assets/js/colorbox/jquery.colorbox.js">
										</script>
										<link rel="stylesheet"
											href="<?php echo Uri::root() ?>media/com_osproperty/assets/js/colorbox/colorbox.css"
											type="text/css" media="screen" />
										<script type="text/javascript">
											jQuery(document).ready(function() {
												jQuery(".propertyphotogroup").colorbox({
													rel: 'colorbox',
													maxWidth: '95%',
													maxHeight: '95%'
												});
									...
									' . $noconflict . '
										...
										<div id="slider1" class="favslider1 margin0">
											<ul class="favs">
												...
												?>
															<li><a class="propertyphotogroup"
																	href="<?php echo Uri::root() ?>images/osproperty/properties/<?php echo $row->id; ?>/<?php echo $photos[$i]->image ?>"><img
																		src="<?php echo Uri::root() ?>images/osproperty/properties/<?php echo $row->id; ?>/medium/<?php echo $photos[$i]->image ?>"
																		alt="<?php echo $photos[$i]->image_desc; ?>"
																		title="<?php echo $photos[$i]->image_desc; ?>" /></a></li>
														<?php
														} else {
														?>
															<li><img src="<?php echo Uri::root() ?>media/com_osproperty/assets/images/nopropertyphoto.png"
																	alt="<?php echo $photos[$i]->image_desc; ?>"
																	title="<?php echo $photos[$i]->image_desc; ?>" /></li>
														<?php
														}
													} else {
														?>
													...
													?>
																<li <?php if ($i > 0) { ?>style="margin-left: 3px;width:120px !important;"
																	<?php } else { ?>style="width:120px !important;; " <?php } ?>><img
																		class="detailwidth" alt="<?php echo $photos[$i]->image_desc; ?>"
										...
										<ul class="attribute-list">
											...
											?>
													<li class="propertyinfoli meta-block">
														<strong><?php echo Text::_('OS_FLOORS') ?>:
														</strong><span><?php echo $row->number_of_floors; ?></span>
													</li>
												<?php
												}
												?>
											...
											?>
												<li class="propertyinfoli meta-block"><?php echo Text::_('OS_RATING') ?>:
													<span><?php echo $row->ratingvalue ?></span>
												</li>
											<?php
											}
											?>
											<li class="propertyinfoli meta-block">
												<strong><?php echo Text::_('OS_CATEGORY') ?>:
												</strong><span><?php echo $row->category_name ?></span>
											</li>
											<?php
											if (count((array)$tagArr) > 0) {
											?>
												<li class="propertyinfoli meta-block"><strong><?php echo Text::_('OS_TAGS') ?>:
													</strong>
													<span>
					...
					<ul class="listingActions-list">
						...
						if ($configClass['show_compare_task'] == 1) {
							...
							?>
								<li class="propertyinfoli">
									...
									<span id="compare<?php echo $row->id; ?>">
										<a onclick="javascript:osConfirmExtend('<?php echo $msg; ?>','ajax_addCompare','<?php echo $row->id ?>','<?php echo Uri::root() ?>','compare<?php echo $row->id ?>','theme1','details')"
							...
							?>
								<li class="propertyinfoli">
									...
									<i class="osicon-list"></i>
									<span id="compare<?php echo $row->id; ?>">
										<a onclick="javascript:osConfirmExtend('<?php echo $msg; ?>','ajax_removeCompare','<?php echo $row->id ?>','<?php echo Uri::root() ?>','compare<?php echo $row->id ?>','theme1','details')"
											href="javascript:void(0)">
						...
						if (($configClass['property_save_to_favories'] == 1) and ($user->id > 0)) {
							...
							?>
								<li class="propertyinfoli">
									<?php
									$msg = Text::_('OS_DO_YOU_WANT_TO_ADD_PROPERTY_TO_YOUR_FAVORITE_LISTSvvv');
									$msg = str_replace("'", "\'", $msg);
									?>

									<i class="osicon-star"></i>
									<span id="fav<?php echo $row->id; ?>">
										<a onclick="javascript:osConfirmExtend('<?php echo $msg; ?>','ajax_addFavorites','<?php echo $row->id ?>','<?php echo Uri::root() ?>','fav<?php echo $row->id; ?>','theme1','details')"
											href="javascript:void(0)" class="_saveListingLink save has icon s_16">
											<?php echo Text::_('OS_ADD_TO_FAVORITESbfdbf'); ?>
										</a>
									</span>
								</li class="propertyinfoli">
							<?php
							...
							?>
								<li class="propertyinfoli">
									<?php
									$msg = Text::_('OS_DO_YOU_WANT_TO_ADD_PROPERTY_TO_YOUR_FAVORITE_LISTSzzz');
									$msg = str_replace("'", "\'", $msg);
									?>

									<i class="osicon-star"></i>
									<span id="fav<?php echo $row->id; ?>">
										<a onclick="javascript:osConfirmExtend('<?php echo $msg; ?>','ajax_removeFavorites','<?php echo $row->id ?>','<?php echo Uri::root() ?>','fav<?php echo $row->id; ?>','theme1','details')"
											href="javascript:void(0)" class="_saveListingLink save has icon s_16">
						...
						?>
							<li class="propertyinfoli">
								...
								<a href="<?php echo Uri::root(); ?>index.php?option=com_osproperty&task=property_showpano&id=<?php echo $row->id ?>&tmpl=component"
									class="reportmodal" rel="{handler: 'iframe', size: {x: 650, y: 420}}">
								...
								<ul class="additional-details clearfix">
									...
									?>
										<li>
											...
											?>
												<span><?php echo $neighbodhood->mins ?> <?php echo Text::_('OS_MINS') ?>
													<?php echo Text::_('OS_BY') ?>
												<?php
						...
						?>
							...
							<li class="propertyinfoli">
								...
								<a href="https://twitter.com/intent/tweet?original_referer=<?php echo $url; ?>&tw_p=tweetbutton&url=<?php echo $url; ?>"
									target="_blank" title="<?php echo Text::_('OS_ASK_YOUR_TWITTER_FRIENDS'); ?>"
									id="link2Listing" rel="canonical">
						...
						<div class="<?php echo $bootstrapHelper->getClassMapping('row-fluid'); ?>">
							...
							?>
								<div class="<?php echo $bootstrapHelper->getClassMapping('span6'); ?>">
									...
									<a href="<?php echo $row->pro_pdf ?>" title="<?php echo Text::_('OS_PROPERTY_DOCUMENT') ?>"
								...
								?>
									<div class="<?php echo $bootstrapHelper->getClassMapping('span3'); ?> documentElement">
										...
										<div class="media-info">
											...
											<a href="<?php echo $fileUrl; ?>" title="<?php echo Text::_('OS_PROPERTY_DOCUMENT') ?>"
												alt="<?php echo Text::_('OS_PROPERTY_DOCUMENT') ?>" target="_blank"
			...
			?>
					<div
						class="detailsBar clearfix <?php echo $bootstrapHelper->getClassMapping('row-fluid'); ?> calendar-detail">
						<div class="<?php echo $bootstrapHelper->getClassMapping('row-fluid'); ?>">
							<div class="<?php echo $bootstrapHelper->getClassMapping('span12'); ?>">
					...
					<div class="<?php echo $bootstrapHelper->getClassMapping('row-fluid'); ?>">
						<div class="<?php echo $bootstrapHelper->getClassMapping('span12'); ?>">
							<div class="shell">
								<fieldset>
									<legend><span
											class="relate_properties_heading"><?php echo Text::_('OS_RELATE_PROPERTY') ?></span>
									</legend>
								</fieldset>
								<?php
								echo $row->relate;
				...
				<div class="detailsBar clearfix">
					<div class="<?php echo $bootstrapHelper->getClassMapping('row-fluid'); ?>">
						<div class="<?php echo $bootstrapHelper->getClassMapping('span12'); ?>">
							<div class="shell">
								<fieldset>
									<legend><span><?php echo Text::_('OS_JCOMMENTS') ?></span></legend>
								</fieldset>
								<?php
