...
class AriUtils
{
	public static function sortAssocArray($data, $key, $dir = 'asc', $cmp = 'string')
	{
	...
	public static function parseValueBySample($str, $sample)
	{
		return static::parseValue($str, gettype($sample));
	}
	
	public static function parseValue($str, $type)
	{
		$retVal = $str;
		switch ($type)
		{
	...
	public static function getValue($val, $emptyValue)
	{
		return !empty($val) ? $val : $emptyValue;
	}
	
	public static function getParam($arr, $name, $defValue = null)
	{
		$retValue = $defValue;
	...
	public static function isRemoteResource($link)
	{
		if (empty($link))
			return false;
	...
	public static function absPath2Url($path)
	{		
	...
	public static function absPath2Relative($path)
	{
		$absPath = str_replace('\\', '/', JPATH_ROOT);
		$path = str_replace('\\', '/', $path);
		if ($absPath != '/')
	...
	public static function getFilteredParam($arr, $name, $defValue = null, $filterMask = 0)
	{
		$param = static::getParam($arr, $name, $defValue);
