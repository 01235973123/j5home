...
class MPFController
{
	...
	public function __construct(MPFInput $input = null, array $config = [])
	{
		...
		$config['default_view'] = $config['default_view'] ?? $config['component'];
	...
	public function display($cachable = false, array $urlparams = [])
	{
		...
		if ($view instanceof MPFViewHtml && $this->app->isClient('site'))
		{
	...
	public function populateModelStates($model)
	{
		...
		if (count($states))
		{
			...
			if ($model->rememberStates)
			{
				...
				if ($this->app->isClient('site') && $Itemid > 0)
				{
					$context = $this->option . '.' . $this->input->get('view', $this->config['default_view']) . '.' . $Itemid . '.';
				}
				else
				{
				...
				$isPostRequest = $this->input->getMethod() === 'POST';

				foreach ($states as $state)
				{
					// Do not remember states on POST requests for states uses choices JS, fix clear icon does not work for choices
					if ($isPostRequest && in_array($state, $model->getChoicesJSStates()))
					{
