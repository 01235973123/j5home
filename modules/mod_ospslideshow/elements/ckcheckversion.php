...
class JFormFieldCkcheckversion extends FormField {
	...
	protected function getLabel() {
		...
		$html = '<div style="' . $styles . '"><img src="' . $this->getPathToElements() . '/images/information.png" style="margin: 0 10px 0 0;" /><b>OSP Slideshow</b> - ' . Text::_('MOD_SLIDESHOWCK_CURRENT_VERSION') . ' : <span class="label">' . $current_version . '</span></div>';
						...
						for (var version in response) {
							if (compareVersions(version,"' . $current_version . '")) {
								...
								if (! jQuery("#'.$this->type.'updatealert").text().length) {
									jQuery("#'.$this->type.'updatealert").append("<span class=\"label label-warning\" style=\"font-size:1em;padding:0.4em;\">' . Text::_('MOD_SLIDESHOWCK_NEW_VERSION_AVAILABLE') . ' : "+latest_version+"</span>");
									jQuery("#'.$this->type.'updatealert").append("<a href=\"http://www.joomdonation.com/en/joomla-extensions/slideshow-ck\" target=\"_blank\" class=\"pull-right btn btn-info\" style=\"font-size:1em;padding:0.2em 0.4em;margin: 0 0 0 5px;\"><i class=\"icon icon-download\"></i>' . Text::_('MOD_SLIDESHOWCK_DOWNLOAD') . '</a>");
								}
	...
	public static function get_current_version() {
		...
		if ($xml_installed = Factory::getXML($file_url)) {
