...
class JFormFieldCkcheckpluginversion extends FormField {
	...
	protected function getLabel() {
		...
		if (! File::exists(JPATH_SITE .'/plugins/system/slideshowckparams/slideshowckparams.xml')) {
			return '';
		}
		...
		if (version_compare($this->element['version'], $current_version) > 0) {
			$html .= '<div id="'.$this->type.'updatealert" style="' . $styles . '">PLUGIN SLIDESHOW CK PARAMS - ' . Text::_('MOD_SLIDESHOWCK_NEED_UPDATE') . '<br />'
				. Text::_('MOD_SLIDESHOWCK_CURRENT_VERSION'). ' : <span class="label">' . $current_version . '</span><br />'
				. Text::_('MOD_SLIDESHOWCK_REQUIRED_VERSION'). ' : <span class="label">' . $this->element['version'] . '</span>'
				. '<a href="'.$this->url.'" target="_blank" class="pull-right btn btn-info" style="font-size:1em;padding:0.2em 0.4em;margin: 0 0 0 5px;"><i class="icon icon-download"></i>' . Text::_('MOD_SLIDESHOWCK_DOWNLOAD') . '</a>'
				. '</div>';
		}
